...
//------------------------------------------------------------------------------------------------------------//
void Matris(int a[][10]) // ساخت ماتریس بازی
{
...
//------------------------------------------------------------------------------------------------------------//
int MokhtasatX(int x) // x مختصات محور
{
	return 2 + 4 * x;
}
//------------------------------------------------------------------------------------------------------------//
int MokhtasatY(int y) // y مختصات محور
{
	return 1 + 3 * y;
}
//------------------------------------------------------------------------------------------------------------//
void Marble(int a[][10]) // کشیدن مهره ها به همراه خانه هایش در صفحه
{
	for (size_t i = 0; i < 8; i++)
	{
		for (size_t j = 0; j < 8; j++)
...
//------------------------------------------------------------------------------------------------------------//
void RemoveMarble(int a[][10], int x, int y, int x_change, int y_change, int &x_rmv, int &y_rmv) // حذف کردن مجاز مهره ها
{
	x_rmv = -1, y_rmv = -1;
...
//------------------------------------------------------------------------------------------------------------//
int EndGame(int a[][10]) // تشخیص پایان بازی
{
	int cnta = 0, cntb = 0;
	for (size_t i = 0; i < 8; i++)
	{
...
//------------------------------------------------------------------------------------------------------------//
void DoingAndMoveCursor(int a[][10]) // خلاصه که همه کارا توشه
{
	srand(time(NULL));
	char ch = _getch();
	...
	while (true)
	{
		...
		else if (ch == 80)
		{
			y++;
			if (y > 7)
			{                                                  // کلیدهای جهت برای جابجایی مکان نما
				y = 0;
			}
			gotoxy(MokhtasatX(x), MokhtasatY(y));
		}
		...
		else if ((ch == 'd' || ch == 'D') && sw == 0 && cnt_score == 0) // حذف توسط کاربر
		{
			if (a[y][x] == 1)
			{
				a[y][x] = 0;
				CnZ[0] = x;
				CnZ[1] = y;
				gotoxy(MokhtasatX(x), MokhtasatY(y));  // توجه شود که کاربر فقط یکبار میتواند حذف کند*
				cerr << " ";
				sw = 1;
			}
		}
		else if ((ch == 'r' || ch == 'R') && sw == 0 && cnt_score == 0) // حذف به صورت تصادفی
		{
			int Rand_X = rand() % 8, Rand_Y = rand() % 8;
			if (a[Rand_Y][Rand_X] == 0)
			{
				if (Rand_X == 0)
				{                                     // توجه شود که کاربر فقط یکبار میتواند حذف کند*
					Rand_X++;
				}
				else if (Rand_Y == 0)
				{
		...
		else if (ch == 13) // حرکت دادن هایی که مجاز است
		{
		...
		else if (ch == 'u' || ch == 'U') // برگشتن به عقب
		{
			if (cnt_score == 0 && sw == 1) //این برای برگشتن به عقب اولیه
			{
				a[CnZ[1]][CnZ[0]] = 1;
			...
			else if (cnt_score > 0) // این برای برگشتن به عقب به جز اولیه
			{
				i--;
		...
		else if (ch == 27) // Esc سیو شدن بازی و برگشت به منو
		{
			Color(0xF0);
			gotoxy(11, 29);
			cerr << "Save Game...";
		...
		if (cnt_score > 5)
		{
			sw_End = EndGame(a); // اینجاست که بازی به آخرش میرسه
			if (sw_End == 0)
			{
				int size;
				Player player[100];

				size = LoadPlayer(player); // اینجا بررسی میشه که آیا قبلا تو فایل چیزی بوده یا نه
				int i;
				for (i = 0; Name[i]; i++)
				{
				...
				SavePlayer(player, size + 1);  // اینجا مشخصات بازیکن سیو میشه
...
//------------------------------------------------------------------------------------------------------------//
void PlayGame() // بازی از اول شروع میشه
{
	Score = 0, Undo = 0;
	int x = 4;
	Frame();
...
//------------------------------------------------------------------------------------------------------------//
void LoadGame() // بازی از جایی که سیو میشه شروع میشه
{
	int a[10][10];
	LoadFile(a);
	TableRow();
...
//------------------------------------------------------------------------------------------------------------//
void Scores() // اینجا بازیکنا یا امتیازاشون نمایش داده میشن
{
	int x = 2, sw = 0;
	char back;
	Player player[100];
	int n;
	Frame();
	n = LoadPlayer(player); // بررسی میکنه بازیکنی هست یا نه
	...
	if (n > 0)
	{                               // اگر بازیکن بود تعدادشو پاس میده
		Color(0xF9);
		gotoxy(x, 1);
...
//------------------------------------------------------------------------------------------------------------//
void SaveFile(int a[][10], char name[], int score, int undo) // سیو میکنه ماتریس بازیو با مشخصات بازیکن
{
...
//------------------------------------------------------------------------------------------------------------//
void LoadFile(int a[][10]) // لود میکنه بالاییو
{
	char name[100];
	int score, undo;
...
//------------------------------------------------------------------------------------------------------------//
void SavePlayer(Player a[], int n) // سیو میکنه مشخصات بازیکن هارو
{
...
//------------------------------------------------------------------------------------------------------------//
int LoadPlayer(Player a[]) // لود میکنه بالاییرو
{
